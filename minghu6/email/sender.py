...
class EmailSender:
    ...
    def __init__(
        self, email_addr, auth_pwd, smtp_addr=None, username=None, logger=None
    ):
    ...
    def guess_smtp_server(email_addr):
        return re.sub(r"(?:.*)@(\w+)(?:\.)(\w+)", r"smtp.\1.\2", email_addr)

    @staticmethod
    def guess_username(email_addr):
        group = re.search(r"\w+(?=@)", email_addr)
    ...
    def send(
        ...
        if not attaches:
            ...
            self._add_attachments(
                msg, bodytext, attaches, bodytextEncoding, attachesEncodings
            )
    ...
    def _add_attachments(
        self, mainmsg, bodytext, attaches, bodytextEncoding, attachesEncodings
    ):
        """
