...
class MailFetcher(MailTool):
    ...
    """
    def __init__(self, popserver=None, popuser=None, pop_passwd=None, hastop=True):
        self.popServer   = popserver
        self.popUser     = popuser
        self.srvrHasTop  = hastop
        self.popPassword = pop_passwd  # ask later if None
    ...
    def connect(self):
        ...
        server = poplib.POP3(self.popServer,
                             timeout=getattr(mailconfig, 'timeout', 20))
    ...
    def decodeFullText(self, messageBytes):
        ...
        """
        text = None
        kinds =  [getattr(mailconfig, 'fetchEncoding', 'utf-8')]            # try user setting first
        kinds += ['ascii', 'latin1', 'utf8', 'cp936']      # then try common types
        ...
        if text == None:
            ...
            commons   = ['ascii', 'latin1', 'utf8', 'cp936']
    ...
    def downloadAllHeaders(self, progress=None, loadfrom=1):
        ...
        if not self.srvrHasTop:                    # not all servers support TOP
            ...
            fetchlimit = getattr(mailconfig, 'fetchlimit', 50)
