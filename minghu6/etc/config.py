...
class ReservedSectionNameError(Exception):
    pass
...
class SmallConfig:
    ...
    def set_section(self, section_name, iterable_obj):
        ...
        if section_name.startswith("_"):
            raise ReservedSectionNameError(
                "`%s` shouldn't start with `_` " % section_name
            )
    ...
    def write_log(
        self, filepath, mode="w", sep=" ", log_id=None, format_func=None, **kwargs
    ):
        """
            ...
            if "b" not in mode:
                ...
                for key, value in self._section_dict.items():
                    if value is not None and key not in [
                        ...
                        fw.write("[%s]\n" % key)
                        [
                            fw.write("{0}\n".format(format_func(key, elem, sep)))
                            for elem in value
                        ]
        ...
        with open(filepath, mode, **kwargs) as fr:
            ...
            for section_name, section_content in self._section_dict.items():
                if not section_name.startswith("_"):
                    self._section_dict[section_name] = [
                        format_func(section_name, line, sep) for line in section_content
                    ]
                else:
