...
class MainWindow_Syn(QtGui.QMainWindow):
    ...
    def setsell_ticket_casualButton(self):
        ...
        def sell_ticket_casual():
            ...
            msg = 'initial tickets number is {0:d}\nseller number is {1:d}\nNow,selling start...\n'.format(
                self.ticket_number_casual.value, pnumber)
            self.sell_ticket_data_queue.put(msg)
            self.processes1 = []
            for i in range(pnumber):
                self.processes1.append(Process(target=sell_ticket_casual_process,
                                               args=(i, self.sell_ticket_data_queue, self.ticket_number_casual),
                                               daemon=True))
    ...
    def setsell_ticket_restrictButton(self):
        ...
        def sell_ticket_restrict():
                ...
                while True:
                    ...
                    with sell_ticket_lock:
                        if (ticket_number_restrict.value <= 0):
                            ...
                            msg = 'process {0:d}: there (are/is) {1:d} tickets available-restrict'.format(id,
                                                                                                          ticket_number_restrict.value)
                            sell_ticket_data_queue.put(msg)
            ...
            msg = 'initial tickets number is {0:d}\nseller number is {1:d}\nNow,selling start...\n'.format(
                self.ticket_number_restrict.value, pnumber)

            self.sell_ticket_data_queue.put(msg)
            self.processes2 = []
            for i in range(pnumber):
                self.processes2.append(Process(target=sell_ticket_restrict_process, args=(
                i, self.sell_ticket_data_queue, self.ticket_number_restrict, self.sell_ticket_lock), daemon=True))
