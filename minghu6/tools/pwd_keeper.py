...
class UsernameUnmatched(Exception):
    pass
...
class PwdKeeper:
    def __init__(self, path, master_password, check_username=False, username=None):
        ...
        self.logger.read_log(
            path, format_func=lambda section_name, line, sep: line.split(sep)
        )
        self.log_id = self.logger.get_section(SmallConfig.LOGID)
        if check_username and self.log_id != username:
            raise UsernameUnmatched(
                "username:%s file_log_id:%s" % (username, self.log_id)
            )

    def flush_read(self):
        self.logger.read_log(
            self.path, format_func=lambda section_name, line, sep: line.split(sep)
        )

    def add_account(self, label, username, password):
        content = self.logger.get_section(label, [])
        encrypt_password = des.encryp_str(
    ...
    def update_account(self, label, username, new_password):
        ...
        content = list(
            filter(lambda x: x[0] != username, self.logger.get_section(label, []))
        )

        encrypt_password = des.encryp_str(
            new_password, self.master_password
        )  # encrypy with des
        content.append([username, encrypt_password])
        self.logger[label] = content
    ...
    def query_account(self, label):
        ...
        return [
            (username, des.decryp_str(encrypt_password, self.master_password))
    ...
    def get_labels(self):
        ...
        return list(
            filter(
                lambda label: not label.startswith("_")
                and self.logger[label] is not None,
            ...
            if input_result.startswith("query") or op == "?":
                ...
                if all_match is None:
                    ...
                    for each_label in pwd_keeper.get_labels():
                        ...
                        label2 = label.lower()
                        if each_label2.startswith(label2) or each_label2.endswith(
                            label2
                        ):
                    ...
                    for item in all_match:
                        try:
                            ...
                            color.print_err(
                                "usrename:{0} passowrd:{1}".format(username, password)
                            )

                            color.print_err(
                                "Warning: Master Password {0} may be Error".format(
                ...
                input_result.startswith("list")
                or input_result == "ls"
                or input_result == "l"
            ):
                [
                    color.print_info(label)
                    for label in pwd_keeper.get_labels()
                    if pwd_keeper
                ]
