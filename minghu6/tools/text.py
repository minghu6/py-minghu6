...
Usage:
  ...
  text convert <filename> <to-charset> [-d] [-i=<info>] [--FC=<from-charset>] [--output=<output>]
...
def convert_file(fn, output, from_charset, to_charset):
    ...
    if from_charset is None:
        ...
        if confidence is None:
            color.print_err("unknown from_charset, " "you must point it explicit")
            return
        elif confidence < 0.7:
            color.print_warn(
                "uncertain from_charset, "
                "maybe %s\n"
...
def read_lntxt(path):
    with open(path, "r") as fr:
        return list(
            filter(lambda ln: len(ln), map(lambda ln: ln.strip(), fr.readlines()))
        )
...
def cli():
    ...
    if arguments["charset"]:
        ...
        to_charset = arguments["<to-charset>"]
