    ...
    def __init__(self, fn, config_dir: Path = CONFIG_DIR):
        ...
        class LexClosureMethod:
            ...
            def __call__(self, other_self, *args, **kwargs):
                return getattr(other_self._config_dict, self.method_name)(
                    *args, **kwargs
                )
...
class TencentOcr:
    def __init__(self):
        ...
        from tencentcloud.ocr.v20181119 import ocr_client  # type: ignore
...
class UnrecognizedTextNameError(Exception):
...
class CangJingGe:
    ...
    def diff_local_upstream(
        local_chapters: LocalChapterType, upstream_chapters: UpstreamChapterType
    ) -> PatchType:
        ...
        for i, (upstream_chapter_name, url) in enumerate(upstream_chapters.items()):
            ...
            if (
                local_chapter_item is None
                or local_chapter_name != upstream_chapter_name
            ):
                offset += 1
    ...
    def pull(self, n=None):
        ...
        chapters_patch = CangJingGe.diff_local_upstream(
            local_chapters, upstream_chapters
        )
        ...
        for chapter_title, (url, index) in patched_chapters_list:
            chapter_content = CangJingGe.trip_text(
                self.fetch_chapter_content(url, None)
            )
    ...
    def fetch_chapter_content(self, page_url, next_part_links):
        ...
        if next_part_links is None:
            links_block = page_content_block.find("center", {"class": "chapterPages"})
            next_part_links = [
                urljoin(self.MAIN_PAGE, atag["href"])
                for atag in links_block.find_all("a")
            ][1:]
        ...
        if next_part_links:
            content = "".join(
                [
                    content,
                    self.fetch_chapter_content(next_part_links.pop(), next_part_links),
                ]
            )
...
def cli():
    ...
    if arguments["list"] and arguments["text-registries"]:
        ...
        for text in list_local_texts():
            ...
            color.print_info(
                f"{text.name} has been cleaned {origin_content_len - cleaned_content_len} chars."
            )
